...
function findUsers (data1, data2) {
    let data_1 = Date.parse(data1);
    let data_2 = Date.parse(data2);
    users.forEach(function (element) {
        let data_expect = Date.parse(element.registrationDate);
        if ((data_expect == data_1) || (data_expect == data_2)) {
            console.log(element);
        }
    });
}

findUsers('09.10.2021', '10.10.2021')
...
// Решения от Анатолия Карповича
function UniqeValue() {
    let unic1 = [];
    users2.forEach (el => {
        let str_el = JSON.stringify(el)
        if (!unic1.includes(str_el))
        unic1.push(str_el)
    })
    return unic1.map(el => JSON.parse(el))

}
// console.log (UniqeValue())

let uniqe = Array.from(new Set(users2.map((item) => (JSON.stringify(item))))).map((item) => (JSON.parse(item)));
console.log (uniqe)
